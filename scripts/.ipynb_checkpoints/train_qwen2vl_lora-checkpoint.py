...
用法:
  python scripts/train_qwen2vl_lora.py \
    ...
    --image-root . \
    --output-dir outputs/lora/chartqa
...
class VLADataset(Dataset):
    """Vision-language QA dataset from project JSONL format."""
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        messages = [
            {
                "role": "system",
                "content": [{"type": "text", "text": "你是一个中文图文问答助手。请根据图片内容简洁准确地回答问题。"}],
            },
            {
                "role": "user",
                "content": [
        ...
        # remove assistant message for labels — model should only generate answer
        ...
        # labels: -100 for prompt part, actual token ids for answer part
...
class VLADataCollator:
    ...
    def __call__(self, features: list[dict]) -> dict:
        ...
        # pixel_values — stack if all same shape, else keep as list
        pvs = [f["pixel_values"] for f in features if f["pixel_values"] is not None]
        if pvs and all(p.shape == pvs[0].shape for p in pvs):
...
def main() -> None:
    ...
    logger.info("显存: %.1f GB", torch.cuda.get_device_properties(0).total_mem / 1e9 if torch.cuda.is_available() else 0)
    ...
    # ---- 数据集 ----
    train_dataset = VLADataset(train_jsonl, image_root, processor, args.max_length)
    val_dataset = VLADataset(val_jsonl, image_root, processor, args.max_length)
